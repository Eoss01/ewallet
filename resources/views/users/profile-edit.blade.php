...
<x-app-layout>
    ...
    <x-page-title title="{{ __('Profile Edit') }}" :breadcrumbs="[['label' => __('Dashboard'), 'url' => route('dashboard')], ['label' => __($user->uid)]]"/>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row g-4 align-items-center">
                        <div class="col-sm">
                            <div>
                                <h5 class="card-title mb-0">{{ __($user->uid) }} {{ __('Profile Edit') }}</h5>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <form action="{{ route('users.profile_update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <x-hidden-input name="version" value="{{ $user->version }}" />

                        <div class="row mb-3">
                            <div class="col-lg-4">
                                <x-text-input name="uid" id="uid" value="{{ $user->uid }}" label="{{ __('UID') }}" placeholder="{{ __('Enter UID') }}" />
                            </div>

                            <div class="col-lg-8">
                                <x-text-input name="name" id="name" value="{{ $user->name }}" label="{{ __('Name') }}" placeholder="{{ __('Enter Name') }}" />
                            </div>
                        </div>

                        <div class="row mb-3">
                            <div class="col-lg-4">
                                <x-phone-input name="phone" id="phone" value="{{ $user->phone }}" label="{{ __('Phone') }}" flag="{{ asset('assets/images/flags/my.svg') }}" country-code="+60" placeholder="{{ __('Enter Phone No. e.g. 137829842') }}" />
                            </div>

                            <div class="col-lg-8">
                                <x-email-input name="email" id="email" value="{{ $user->email }}" label="{{ __('Email') }}" placeholder="{{ __('Enter Email') }}" />
                            </div>
                        </div>
